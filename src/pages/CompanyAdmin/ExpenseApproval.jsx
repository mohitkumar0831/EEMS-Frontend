...
export const ExpenseApproval = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (expData.success && empData.success) {
        ...
        // Only show expenses submitted by Manager, Finance, or Auditor roles
        // (i.e. those who route their claims directly to CompanyAdmin)
        const allowedRoles = ['manager', 'finance', 'auditor'];
        const adminId = currentUser?._id || currentUser?.id;
        const filtered = mapped.filter(exp => {
          const isFromAllowedRole = allowedRoles.includes(exp.employeeRole);
          const isAssignedToMe = exp.assignedManagerId === adminId;
          return isFromAllowedRole || isAssignedToMe;
        });

        setAllExpenses(filtered);

        if (selectedExpense) {
          const updated = filtered.find(e => e._id === selectedExpense._id);
  ...
  const pendingPayouts = allExpenses.filter(e => e.status === 'Manager Approved' || e.status === 'Finance Approved' || e.status === 'Submitted');
          ...
          try {
            ...
            if (verifyData.success) {
              ...
              setSlipData({
                ...
                managerName: 'Company Admin Approver',
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1">
        <h3 className="text-xl font-bold bg-gradient-to-r from-slate-100 to-slate-300 bg-clip-text text-transparent">Company Expense Approvals</h3>
        <p className="text-slate-400 text-xs leading-relaxed">
          Verify approvals, review attached invoices, and disburse corporate expense payouts via corporate gateways.
        </p>
      </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex flex-col items-end gap-1.5">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[8px] font-bold tracking-wider uppercase shrink-0 ${exp.status === 'Paid'
                          ...
                          {exp.status === 'Paid' ? 'PAID / SETTLED' : 'PENDING'}
                        </span>
                      </div>
            ...
            <div className="flex flex-col gap-4 text-xs flex-grow">
              ...
              {selectedExpense.comments && selectedExpense.comments.length > 0 ? (
                ...
                <div className="p-3 bg-indigo-500/5 border border-indigo-500/10 text-indigo-400 rounded-2xl flex items-center gap-2">
                  ...
                  <span className="text-[10px]">L1 Signature verified and validated.</span>
                </div>
              )}
